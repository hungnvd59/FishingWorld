...
<body>
	...
	<form action="searchDouble" method="get">
		...
		<c:forEach var="category" items="${categories}">
			<input type="radio" value="${category.categoryId}" name="cateID" /> ${category.categoryName} <br>
		</c:forEach>
	...
	<table style="border: 1px solid; margin-bottom: 10px;">
		...
		<c:forEach var="category" items="${categories}">
			<tr>
				...
				<td><a href="products?cateID=${category.categoryId}">${category.categoryName}</a></td>
			</tr>
		</c:forEach>
	</table>
	<c:choose>
